...
public class IntervalCollectionTests : TestCaseTestsBase
{
    ...
    {
        var inputIntervals = new List<Interval<int, bool>>(GetIntervals(input).OrderBy(i => i));
        Interval<int, IReadOnlyList<Interval<int, bool>>>[] slices = inputIntervals.Slice().ToArray();
    ...
    {
        var inputIntervals = GetIntervals(input).ToList();
        Interval<int, IReadOnlyList<Interval<int, bool>>>[] slices = inputIntervals.Merge(behavior).ToArray();
        CompareInputAndOutput(inputIntervals, output, outputExplanation, slices);
    }

    private static void CompareInputAndOutput(List<Interval<int, bool>> inputRanges, string output, string outputExplanation, Interval<int, IReadOnlyList<Interval<int, bool>>>[] resultsOfOperation)
    {
        // Disect explanation-string
        var explanation = new Dictionary<string, List<Interval<int, bool>>>();
        ...
        var outputRanges = new List<Interval<int, IReadOnlyList<Interval<int, bool>>>>();
        int index = 0;
        while (index < output.Length)
        {
            ...
            else
            {
                ...
                outputRanges.Add(new Interval<int, IReadOnlyList<Interval<int, bool>>>(startIndex, endIndex, explanation[output.Substring(startIndex, 1)]));
            }
        }
        ...
        {
            Assert.That(resultsOfOperation[sliceIndex], Is.EqualTo(outputRanges[sliceIndex]));
            Assert.That(outputRanges[sliceIndex].Tag, Is.EquivalentTo(resultsOfOperation[sliceIndex].Tag));
        }
    }
    ...
    {
        IEnumerable<Interval<int, bool>> input = Array.Empty<Interval<int, bool>>();
        IEnumerable<Interval<int, IReadOnlyList<Interval<int, bool>>>> output = input.Slice();
    ...
    {
        IEnumerable<Interval<int, bool>>? collection = null;
